...
export function cacheMiddleware(cache: ICache, ttl?: number | string): Middleware {
  return async function cacheIntercept(event: Record<string, unknown>, ctx: Record<string, unknown>, next: CallableFunction) {
    if (!event?.call || !event?.methodDesc) throw TypeError('Cache Middleware can work only with RPC methods, not the Composer!')

    const { target, descriptor } = event.methodDesc as MethodDescription;
    const cacheKey = callToKey(event.call as Call)
    const stored = await cache.get(cacheKey).catch(e => console.error(`Failed at read cache "${cacheKey}"\n`, e))
